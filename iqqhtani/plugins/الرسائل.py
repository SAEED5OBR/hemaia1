...
def build_keyboard(buttons):
    ...
    return keyb
@iqqhtani.on(admin_cmd(pattern="وقتيه (\d*) ([\s\S]*)"))
async def _(event):
...
async def all_groups_id(cat):
    ...
    return catgroups
@iqqhtani.on(admin_cmd(pattern="ازاله التوجيه(?: |$)(.*)"))
async def _(event):
    try:
        await event.delete()
...
@iqqhtani.on(admin_cmd(pattern=f"{ALLGROUB} ?(.*)$"))
...
async def getTranslate(text, **kwargs):
    ...
    for _ in range(10):
        try:
            ...
            await sleep(0.1)
    return result
@iqqhtani.iq_cmd(incoming=True, groups_only=True)
async def _(event):
    if not CHAT_FLOOD:
        return
...
@iqqhtani.on(admin_cmd(pattern=f"{ALLPRIVATE} ?(.*)$"))
...
async def _iq(iqqhtani):
    ...
    if not (iq or iq.isdigit()):
        ...
        try:
            ...
            try:
                iq = await iqqhtani.ban_time(iq)
            except BaseException:
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await iqqhtani.edit(f"**تم بدء وضع الكتابة الوهمية لـ {iq} من الثوانـي**")
    async with iqqhtani.client.action(iqqhtani.chat_id, "typing"):
        await asyncio.sleep(iq)
@iqqhtani.on(admin_cmd(pattern="تكرار (.*)"))
...
@iqqhtani.on(admin_cmd(pattern="مؤقته (\d*) ([\s\S]*)"))
async def selfdestruct(destroy):
    ...
    await sleep(ttl)
    await smsg.delete()
def iqqhtaniveFile(input_file_name):
...
@iqqhtani.on(admin_cmd(pattern="ترحيب(?:\s|$)([\s\S]*)"))
...
@iqqhtani.on(admin_cmd(pattern="ترحيباتي(?: |$)(.*)"))
...
@iqqhtani.on(admin_cmd(pattern="رساله الترحيب السابقه (تشغيل|ايقاف)$"))
async def del_welcome(event):
...
@iqqhtani.on(admin_cmd(pattern="موقع(?: |$)(.*)"))
async def gps(event):
...
async def ocr_iqqhtani(filename, overlay=False,api_key="K88942796188957", language="eng"):
    ...
    return r.json()
@iqqhtani.on(admin_cmd(pattern="فيديو وهمي(?:\s|$)([\s\S]*)"))
async def _iq(iqqhtani):
    iq = iqqhtani.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await iqqhtani.edit(f"**تم بدء وضع فيديو وهمي لـ {iq} من الثوانـي**")
    async with iqqhtani.client.action(iqqhtani.chat_id, "record-video"):
        await asyncio.sleep(iq)
@iqqhtani.on(admin_cmd(pattern="ازعاج(?: |$)(.*)"))
...
async def echo(event):
    ...
    if input_str:
        ...
        try:
            ...
            await edit_or_reply(event, " ◈ ⦙   تـم ايقاف وضـع الازعاج على الجميع بنجاح ✅**")
@iqqhtani.on(admin_cmd(pattern="المزعجهم( -a)?$"))
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
    ...
    if not (iq or iq.isdigit()):
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await iqqhtani.edit(f"**تم بدء وضع صوت وهمي لـ {iq} من الثوانـي**")
    async with iqqhtani.client.action(iqqhtani.chat_id, "record-audio"):
        await asyncio.sleep(iq)
@iqqhtani.on(admin_cmd(pattern="تنظيف(?:\s|$)([\s\S]*)"))
...
@iqqhtani.on(admin_cmd(pattern="لعب وهمي(?:\s|$)([\s\S]*)"))
async def _iq(iqqhtani):
    iq = iqqhtani.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await iqqhtani.edit(f"**تم بدء وضع لعب وهمي لـ {iq} من الثوانـي**")
    async with iqqhtani.client.action(iqqhtani.chat_id, "game"):
        await asyncio.sleep(iq)
@iqqhtani.iq_cmd(pattern="قرائه(?:\s|$)([\s\S]*)",)
async def ocriq(iqqhtanievent):
    iqqevent = await edit_or_reply(iqqhtanievent, "جاري قرائه الصوره ...")
    if not os.path.isdir(Config.TEMP_DIR):
        os.makedirs(Config.TEMP_DIR)
    lang_code = iqqhtanievent.pattern_match.group(1)
    downloaded_file_name = await iqqhtanievent.client.download_media(await iqqhtanievent.get_reply_message(), Config.TEMP_DIR)
    test_file = await ocr_iqqhtani(filename=downloaded_file_name, language=lang_code)
    try:
...
@iqqhtani.on(admin_cmd(pattern="اضف رد ([\s\S]*)"))
...
@iqqhtani.on(admin_cmd(pattern="موقع وهمي(?:\s|$)([\s\S]*)"))
async def _iq(iqqhtani):
    iq = iqqhtani.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await iqqhtani.edit(f"**تم بدء وضع موقع وهمي لـ {iq} من الثوانـي**")
    async with iqqhtani.client.action(iqqhtani.chat_id, "location"):
        await asyncio.sleep(iq)
@iqqhtani.on(admin_cmd(pattern="جميع الردود(?: |$)(.*)"))
...
@iqqhtani.on(admin_cmd(pattern="مسح رد ([\s\S]*)"))
...
@iqqhtani.on(admin_cmd(pattern="مسح جميع الردود(?: |$)(.*)"))
...
@iqqhtani.on(admin_cmd(pattern="تكلم(?:\s|$)([\s\S]*)"))
async def _(event):
    input_str = event.pattern_match.group(1)
    start = datetime.now()
    reply_to_id = await reply_id(event)
...
@iqqhtani.on(admin_cmd(pattern=f"{FOTOSECRET}(?: |$)(.*)"))
async def iq(event):
  ...
  await bot.send_file('me', pic, caption=f"""**الصـورة الوقتيه ✅**\- So : @iqqhtani""")
  await event.delete()
@iqqhtani.on(admin_cmd(pattern="تاريخ الرساله(?: |$)(.*)"))
async def _(event):
...
@iqqhtani.on(admin_cmd(pattern="جهه اتصال وهمي(?:\s|$)([\s\S]*)"))
async def _iq(iqqhtani):
    iq = iqqhtani.pattern_match.group(1)
    if not (iq or iq.isdigit()):
        iq = 100
        ...
        try:
            ...
            try:
                ...
                return await event.edit("**اكتب الامر بشكل صحيح**")
    await iqqhtani.edit(f"**تم بدء وضع جهه اتصال وهمي لـ {iq} من الثوانـي**")
    async with iqqhtani.client.action(iqqhtani.chat_id, "contact"):
        await asyncio.sleep(iq)
@iqqhtani.on(admin_cmd(pattern="ماركدون(?:\s|$)([\s\S]*)"))
async def _(event):
    reply_to_id = await reply_id(event)
    ...
    await event.delete()
@iqqhtani.on(admin_cmd(pattern="حذف(?:\s|$)([\s\S]*)"))
async def iq(cloneiq):
    chat = await cloneiq.get_input_chat()
    msgs = []
    count = 0
    ...
    await sleep(5)
    await hi.delete()
@iqqhtani.on(admin_cmd(pattern="معرفات 100(?: |$)(.*)"))
...
@iqqhtani.on(admin_cmd(pattern="معرفات 200(?: |$)(.*)"))
async def iq(iqqhtani):
    mentions = iqqhtani.text[8:]
    chat = await iqqhtani.get_input_chat()
    ...
    await iqqhtani.client.send_message(iqqhtani.chat_id, mentions)
    await iqqhtani.delete()
@iqqhtani.on(admin_cmd(pattern="تاك 200(?: |$)(.*)"))
async def iq(iqqhtani):
    mentions = iqqhtani.text[8:]
    chat = await iqqhtani.get_input_chat()
    async for x in iqqhtani.client.iter_participants(chat, 200):
...
@iqqhtani.iq_cmd(pattern="(ازاله الخلفيه بالملصق|ازاله الخلفيه)(?:\s|$)([\s\S]*)",)
async def remove_iq(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        try:
            ...
            response = iqqhtaniveFile(file_name)
            os.remove(file_name)
    elif input_str:
        catevent = await edit_or_reply(event, "إزالة خلفية هذه الوسائط")
        response = iqqhtaniveURL(input_str)
    else:
    ...
    remove_bg_image = "iqqhtani.png"
    if "image" in contentType:
        with open("iqqhtani.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await edit_delete(catevent, f"`{response.content.decode('UTF-8')}`", 5)
        return
    if cmd == "ازاله الخلفيه بالملصق":
        file = convert_tosticker(remove_bg_image, filename="iqqhtani.webp")
    ...
    await catevent.delete()
@iqqhtani.on(admin_cmd(pattern="ابلاغ الادمنيه(?: |$)(.*)"))
async def iq(event):
...
async def on_new_message(event):
    ...
    for snip in snips:
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            break
@iqqhtani.on(admin_cmd(pattern="خط ملصق ?(?:(.*?) ?; )?([\s\S]*)"))
async def sticklet(event):
    ...
    image_stream.name = "iqqhtani.webp"
    image.save(image_stream, "WebP")
    image_stream.seek(0)
    await event.client.send_file(        event.chat_id,        image_stream,        caption="iqqhtani's Sticklet",        reply_to=reply_to_id,    )
    try:
        os.remove(FONT_FILE)
    except BaseException:
        pass
@iqqhtani.on(admin_cmd(pattern="ضفدع(?:\s|$)([\s\S]*)"))
...
@iqqhtani.on(admin_cmd(pattern="منع(?:\s|$)([\s\S]*)"))
async def _(event):
    text = event.pattern_match.group(1)
...
@iqqhtani.on(admin_cmd(pattern="الغاء منع(?:\s|$)([\s\S]*)"))
async def _(event):
    text = event.pattern_match.group(1)
...
@iqqhtani.on(admin_cmd(pattern="قائمه المنع$"))
async def _(event):
...
@iqqhtani.on(admin_cmd(pattern="مسح(\s*| \d+)$"))
async def iq(msg1):
    ...
    if iq_src:
        ...
        await msg1.delete()
@iqqhtani.on(admin_cmd(pattern="تاك بالكلام ([\s\S]*)"))
async def iq(event):
...
@iqqhtani.on(admin_cmd(pattern="لافته(?:\s|$)([\s\S]*)"))
...
@iqqhtani.on(admin_cmd(pattern="نسخ(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
...
@iqqhtani.on(admin_cmd(pattern="غمق(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
    if nshkh.reply_to_msg_id:
        ...
        await nshkh.edit(" ◈ ⦙   قم  برد على الرساله ** ")
@iqqhtani.on(admin_cmd(pattern="ينسخ(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
    if nshkh.reply_to_msg_id:
        ...
        await nshkh.edit(" ◈ ⦙   قم  برد على الرساله ** ")
@iqqhtani.on(admin_cmd(pattern="خط سفلي(?: |$)(.*)"))
async def iq(nshkh):
    if nshkh.fwd_from:
        return
    if nshkh.reply_to_msg_id:
