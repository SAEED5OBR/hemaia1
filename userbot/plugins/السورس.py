...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ سورس كوبرا- Telethon-Arabe ، 🕸  ❭ :"
...
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**☭︙   تم تحديث سورس كوبرا ↻**")
    sandy = await edit_or_reply(event , "☭︙  جـاري تـحديـث سورس كوبرا 🔄\n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم" ,)
    try:
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**☭︙  📑 يرجى متابع قناه السورس تجدها هنا : @VFF35")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        if query.startswith("تنصيب") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- قناة السورس", "https://t.me/VFF35"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@telethon-Arab/generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/cobarsyr/-COBRA"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/QABNADLIB"),]]
...
async def iq(event):
    await edit_or_reply(event, """✐  ⦗ اوامـر سـورس كوبرا ⦘
                                             ┉┉┉┉┉┉×┉┉┉┉┉
〖`.م1`〗⏎  اوامر الحساب 1
〖`.م2`〗⏎  اوامر الحساب 2
〖`.م3`〗⏎  اوامر الحساب 3
