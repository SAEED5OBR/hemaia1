...
IQTHONPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/a0d61c571ae7e018c0535.mp4"
...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/a0d61c571ae7e018c0535.mp4"
...
.◈￤ الاسم  {my_mention}  𓇡.
.◈￤ الوقت  {TM}  𓇡.
.◈￤ وقت التشغيل  {uptime}  𓇡.
.◈￤ بوتك  {tg_bot}  𓇡.
.◈￤ بنق  {ping}  𓇡.
.◈￤ نسخة رقم 7.5  𓇡.
.◈￤ السورس  : @ghtanisaeed  𓇡.
.--------------- 𓇡."""
...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ سورس القحطاني- Telethon-Arabe ، 🕸  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"☭︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس القحطاني @ghtanisaeed :**\n\n{o}")
...
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**☭︙   تم تحديث سورس القحطاني ↻**")
    sandy = await edit_or_reply(event , "☭︙  جـاري تـحديـث سورس القحطاني 🔄\n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم" ,)
    try:
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**☭︙  📑 يرجى متابع قناه السورس تجدها هنا : @ghtanisaeed")
...
async def iq(event):
    await edit_or_reply(event, """✐  ⦗ اوامـر سـورس القحطاني ⦘
                                             ┉┉┉┉┉┉×┉┉┉┉┉
〖`.م1`〗⏎  اوامر الحساب 1
〖`.م2`〗⏎  اوامر الحساب 2
〖`.م3`〗⏎  اوامر الحساب 3
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqthon(iqthon):
        ...
        query = iqthon.text
        await bot.get_me()
        if query.startswith("اوامر القروب(?: |$)(.*)") and iqthon.query.user_id == bot.uid:
