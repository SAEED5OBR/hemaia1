...
hanhi = [    "تف عليك ياخايس",    "كرامتك وين ياقندره",    "تعال حيوان انت اليوم تنهق ",    "هاه اخي ؟",    "واحد حيوان ومصلحه عوفه ",    "لك حيوان حيوان استقر لك",
...
@iqthon.on(admin_cmd(pattern="نسبه الخرفنه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    iqth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    iqt = random.choice(arb)
    await edit_or_reply(mention, f"⌔︙ نسبه الخرفنه لـ [{iqth}](tg://user?id={user.id}) هـي {iqt} 🤡 🏳️‍🌈.")
...
@iqthon.on(admin_cmd(pattern="رفع جنيه(?:\s|$)([\s\S]*)"))
async def iq(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
       return
...
async def iq(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚹 ¦ المستخدم ⪼ • ** [{iqth2}](tg://user?id={user.id}) \n ☑️ **¦ تم رفعه مرتبط  👩‍❤️‍💋‍👨. .** \n**🤵‍♂️ ¦ بواسطه  :** {my_mention} ")
@iqthon.on(admin_cmd(pattern="رفع خروف(?:\s|$)([\s\S]*)"))
async def iq(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@iqthon.on(admin_cmd(pattern="رفع مسطول(?:\s|$)([\s\S]*)"))
async def iq(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
